...
typedef struct fileops_s {
  ...
  void     (*file_rename)(path_t *path, cbmdirent_t *oldname, uint8_t *newname);
} fileops_t;
...
#define file_rename(path,old,new) ((pgmcall(partition[(path)->part].fop->file_rename))(path,old,new))
